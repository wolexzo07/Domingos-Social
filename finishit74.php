...
// Modification started for php 7.4 get_magic_quotes_gpc() deprecated
...
$chk = @trim($chk);
$chk = htmlspecialchars($chk);
// Modification started for get_magic_quotes_gpc() deprecation in php 7.4
/****
if(get_magic_quotes_gpc()){
$chk = stripslashes($chk);
}***/
...
function x_clean($chk){
		...
		// modification for php 7.4 get_magic_quotes_gpc() deprecated
		/****
		if(get_magic_quotes_gpc()){
		$chk = stripslashes($chk);
		}***/
...
function x_sanitize($input){
    if (is_array($input)) {
        ...
        /***
		Modification due to get_magic_quotes_gpc() deprecation
		if (get_magic_quotes_gpc()) {
...
// Importing paystack payment functions
