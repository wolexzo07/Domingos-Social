	...
	$userid = x_clean($_SESSION["XELOW_DOMINGOS_USER_ID"]);
		$cmd = "SELECT * FROM userdatabank
					...
					$count = x_querycount($cmdcount);
						if($count > 0){
							?>
					<h4 class="followbadge"><i class="fa fa-database"></i>&nbsp;&nbsp;&nbsp;PEOPLE <font class="colorg">YOU ARE FOLLOWING</font></h4>
					<hr/>
							<?php
						...
						<div  style="height:280px;" class="panel-body panelb">
						<img src="<?php
						if($upload != ""){
							echo "../".$upload;
						}else{
						...
						<button style="float:right;margin-top:10pt;" class="btn btn-primary btn-sm clicktofollow<?php echo sha1($id);?>"><i class="fa fa-unlink"></i> &nbsp;unFollow </button>
						</form>
					...
					<div class="showmefollowingmsg<?php echo sha1($id);?>"></div>
						...
						$(".showmefollowingmsg<?php echo sha1($id);?>").html("");
						$(".clicktofollow<?php echo sha1($id);?>").html("<i class='fa fa-link'></i>&nbsp; Follow");
							...
							var dataString = "";
							 $.ajax({
								   type: "POST",
								   url: "check_your_followers_people",
								   data: dataString,
								   cache: false,
								...
								// Refreshing followers record ended
