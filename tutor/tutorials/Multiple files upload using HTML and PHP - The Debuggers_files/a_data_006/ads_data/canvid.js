        ...
        if (hasCanvas()) {
            ...
            loadImages(_opts.videos, function(err, images) {
                ...
                control.play = function(key, fps) {
                    ...
                    for (var f = 0; f <= opts.frames; f++) {
                        fx.push(Math.floor(f % opts.cols) * frameWidth);
                        fy.push(Math.floor(f / opts.cols) * frameHeight);
                    }
                    ...
                    function frame() {
                        if (!wait) {
                            drawFrame(fx[curFrame], fy[curFrame]);
                            curFrame = (+curFrame + 1);
                            if (curFrame > opts.cols)
